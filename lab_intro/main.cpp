...
/**
 * This function is made to make an image black and white. That is, each pixel in
 * the returned image will only consist of the colors white, greys, and black.
 *
 * @param firstImage   The image that will be converted to black and white
 *
 * @return  The resulting black and white photo.
 */
PNG blackWhite(PNG firstImage)
{
    for (size_t yi = 0; yi < firstImage.height(); yi++) {
        for (size_t xi = 0; xi < firstImage.width(); xi++) {
		firstImage(xi, yi)->red = (firstImage(xi, yi)->red + firstImage(xi, yi)->blue + firstImage(xi,yi)->green)/3;
                firstImage(xi, yi)->green = firstImage(xi, yi)->red;
                firstImage(xi, yi)->blue = firstImage(xi, yi)->red;
        }
    }
    return firstImage;
}
...
/**
 * This function flips the image along the y axis. That is, each pixel in the
 * returned image that was previously on the left half of the image will now
 * be in the right half, and vice versa.
 *
 ...
 */
PNG flipAcrossY(PNG firstImage, PNG secondImage)
{
    for (size_t yi = 0; yi < firstImage.height(); yi++) {
        for (size_t xi = 0; xi < firstImage.width(); xi++) {
		*firstImage(xi, yi) = *secondImage(firstImage.width()-xi-1, yi);
        }
    }
    return firstImage;
}
...
int main()
{
    ...
    image.writeToFile("output1.png");
    image = blackWhite(image);
    image = flipAcrossY(image, image);
    image.writeToFile("output2.png");
