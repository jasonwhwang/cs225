...
class KDTree
{
  ...
  private:
    ...
    KDTree(const vector<Point<Dim>>& newPoints, int lowIndex, int highIndex, int d);

    Point<Dim> getPoint(const vector<Point<Dim>>& newPoints, int index, int d);

    void sort(vector<Point<Dim>>& newPts, int d);

    Point<Dim> findNearestNeighbor(const Point<Dim>& query, const Point<Dim>& currentBest, int lowIndex, int highIndex, int d) const;
