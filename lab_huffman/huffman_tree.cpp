...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(size_t i = 0; i < frequencies.size(); i++){
        newNode = new TreeNode(frequencies[i]);
        singleQueue.push(newNode);
    }
    while(singleQueue.size() + mergeQueue.size() > 1 ){
            leaf1 = removeSmallest(singleQueue,mergeQueue);
        if(singleQueue.size() + mergeQueue.size() > 0){
            leaf2 = removeSmallest(singleQueue,mergeQueue);
        }

        newNode = new TreeNode(leaf1->freq.getFrequency() + leaf2->freq.getFrequency());
        newNode->left = leaf1;
        newNode->right = leaf2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()){
        if(bfile.getNextBit()){
            n = new TreeNode(Frequency(bfile.getNextByte(),0));
        } else {
            n = new TreeNode(Frequency(0));
