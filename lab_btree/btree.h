...
size_t insertion_idx(const std::vector<T>& elements, const C& val, int lowIndex, int highIndex)
{
    if(elements[(highIndex+lowIndex)/2] < val && elements[(highIndex+lowIndex)/2 + 1] > val) {
        return (highIndex+lowIndex)/2 + 1;
    } else if(elements[(highIndex+lowIndex)/2] == val) {
        return (highIndex+lowIndex)/2;
    } else if(elements[(highIndex+lowIndex)/2] > val) {
        return insertion_idx(elements, val, lowIndex, (highIndex+lowIndex)/2);
    } else {
        return insertion_idx(elements, val, (highIndex+lowIndex)/2+1, highIndex);
    }
}
...
*/
    ...
    return insertion_idx(elements,val,0,elements.size()-1);
