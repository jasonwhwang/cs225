...
{
    animation a;
    OrderingStructure<int> xval, yval;
    std::vector<std::vector<bool>> processed;
    processed.resize(img.height(), std::vector<bool>(img.width(),false));
    int frames = 0, xCoor, yCoor, distance, redOrig, greenOrig, blueOrig;
    RGBAPixel pixelCurr;

    xval.add(x);
    yval.add(y);
    redOrig = img(x,y)->red;
    greenOrig = img(x,y)->green;
    blueOrig = img(x,y)->blue;

    while(!xval.isEmpty() && !yval.isEmpty()){
        xCoor = xval.remove();
        yCoor = yval.remove();
        pixelCurr = *img(xCoor,yCoor);
        distance = pow(pixelCurr.red-redOrig,2) + pow(pixelCurr.green-greenOrig,2) + pow(pixelCurr.blue-blueOrig,2);

        if(!processed[xCoor][yCoor] && distance <= tolerance){
            *img(xCoor,yCoor) = fillColor(xCoor, yCoor);
            
            if(xCoor+1 < (int)img.width()){
                xval.add(xCoor+1);
                yval.add(yCoor);
            }
            if(yCoor+1 < (int)img.height()){
                xval.add(xCoor);
                yval.add(yCoor+1);
            }
            if(xCoor-1 >= 0){
            ...
            frames++;
            if(frames%frameFreq == 0) { a.addFrame(img); }
            processed[xCoor][yCoor] = true;
        }
    }
