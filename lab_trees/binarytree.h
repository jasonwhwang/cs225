...
class BinaryTree
{
  ...
  public:
    ...
    /**
     *  Mirror helper function. Takes in node, swaps left right pointers.
     */ 
    void mirror(Node* n);
    ...
    /**
     * isOrdered helper function. Returns is tree is ordered or not.
     */
    bool isOrdered(Node* n) const;
    T isOrdered_maxRight(Node* n) const;
    T isOrdered_minLeft(Node* n) const;
  ...
  private:
    ...
    void printPaths(Node* n, std::vector<T> &v);
