...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string u = ll.url;
        string k = ll.customer.substr(1, ll.customer.size()-3);

        if( whenVisitedTable.keyExists(k+u)) {
            if(whenVisitedTable[k+u] < ll.date) {
                whenVisitedTable[k+u] = ll.date;
            }
        } else {
            whenVisitedTable.insert(k+u, ll.date);
        }

        if(!pageVisitedTable.keyExists(u)) {
            uniqueURLs.push_back(u);
        }
        pageVisitedTable[u] = true;
...
{
    string u = url;
    string k = customer.substr(1, customer.size()-3);
    return whenVisitedTable.keyExists(k+u);
}
                                  ...
                                  const string& url) const
{
    string u = url;
    string k = customer.substr(1, customer.size()-3);
    return whenVisitedTable.find(k+u);
}
